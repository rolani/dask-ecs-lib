...
tags_ = {"app": "dask_ecs_lib", "createdWhen": str(datetime.now())}


# function to expose params
def runTask(
    mode, func, num_workers=3, worker_vcpu=4096, worker_memory=8192, return_result=False
):
    """
    ...
    :param worker_vcpu -> int -> The amount of CPU to request for worker tasks in milli-cpu
    (1/1024). Valid options (1024, 2048, 4096, etc). Default 4096
    ...
    if mode == "local":
        ...
        createFargateContext(
            func, num_workers, dask_image, worker_vcpu, worker_memory, return_result
        )
    else:
        logger.info("mode has to be local or fargate")


def createFargateContext(
    func, num_workers, dask_image, worker_vcpu, worker_memory, return_result
):
    """
    ...
    try:
        ...
        with FargateCluster(
            ...
            logger.info(
                "cluster startup time is {:.3f} seconds".format(time.time() - begin)
            )

            logger.info(
                "use the following url to visit dask dashboard: {}".format(
                    svutils.parse_url(str(cluster))
                )
            )
            start = time.time()
        ...
        if cluster_ran:
            ...
            logger.info(
                "runtime of the program is {:.3f} seconds".format(elapsed_time_)
            )
...
def createLocalContext(func, num_workers, return_result):
    ...
    with LocalCluster(n_workers=num_workers, threads_per_worker=1) as local_cluster:
        with Client(local_cluster) as _:
            start = time.time()
            if return_result:
                logger.info("Running in return mode")
                return func()
